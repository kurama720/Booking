...
export const validationSchema = yup.object().shape({
    first_name: yup.string().matches(/^[a-zA-Z]+$/, "Must be only letters")
        ...
        .max(32, 'The maximum letters be 32!').required('First Name is required!'),
    ...
    password: yup.string().matches(/^((?=.*\d)(?=.*[a-zA-Z]).{8,})$/,
        "Password must be at least 8 characters, letters and numbers!"
