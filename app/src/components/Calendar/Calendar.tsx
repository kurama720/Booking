...
const Calendar =
    ({
       ...
       setUserBookingDate,
       calendarPopUpStatus,
       userBookingDate}: CalendarMenuProps) => {
  const [placeholderStatus, setPlaceholderStatus] = useState<string>('Check in – Check out')
  ...
  useEffect(() => {
    if (!!date[0] && !!date[1]) {
      ...
      setPlaceholderStatus(`${firstDateDay}/${firstDateMonth}/${firstDateYear} – ${secondDateDay}/${secondDateMonth}/${secondDateYear}`)
      setUserBookingDate({
      ...userBookingDate,
